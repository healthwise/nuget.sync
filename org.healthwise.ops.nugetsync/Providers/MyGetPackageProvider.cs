...
{
    public class MyGetPackageProvider : IPackageProvider
    {
        ...
        public MyGetPackageProvider(string repositoryUrl, string writeToken, string readUsername, string readPassword)
        {
                    ...
                    foreach (var entryElement in odata.Root.Elements(AtomNamespace + "entry"))
                    {
                        ...
                        try
                        {
                            var packageDefinition = new PackageDefinition
                            {
                                ...
                                LastEdited =
                                    DateTime.Parse(
                                        propertiesElement.Element(DataServicesNamespace + "LastEdited").Value),
                        ...
                        {
                            Log.Logger.Error("An Error Occured.", ex);
                            throw;
                        }
                    }
