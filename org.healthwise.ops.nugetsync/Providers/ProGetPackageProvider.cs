...
{
    public class ProGetPackageProvider : IPackageProvider
    {
        ...
        public ProGetPackageProvider(string repositoryUrl, string writeToken, string readUsername, string readPassword)
        {
        ...
        public async Task<ICollection<PackageDefinition>> GetPackages(DateTime since)
        {
            ...
            var result = await GetPackagesFromUrl(
                $"{_repositoryUrl}/Packages?$select=Id,Version,Published");
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                ...
                using (var odataStream = await response.Content.ReadAsStreamAsync())
                {
                    ...
                    foreach (var entryElement in odata.Root.Elements(AtomNamespace + "entry"))
                    {
                        ...
                        var titleElement = entryElement.Element(AtomNamespace + "title");
                        var updatedElement = entryElement.Element(AtomNamespace + "updated");
                        ...
                        try
                        {
                            ...
                            {
                                PackageType = "nuget",
                                PackageIdentifier = titleElement.Value,
                                PackageVersion = propertiesElement.Element(DataServicesNamespace + "Version").Value,
                                LastEdited = DateTime.Parse(updatedElement.Value),
